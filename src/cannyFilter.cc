...
void Image::applyCannyFilter(unsigned int lowLimit, unsigned int highLimit) {
	...
	for (unsigned int i = 1; i < height - 1; i++) {
		...
		for (unsigned int j = 1; j < width - 1; j++) {
			...
			float SXvalue = (-1 * pixels[i - 1][j - 1] + 1 * pixels[i - 1][j + 1] + -2 * pixels[i][j - 1] + 2 * pixels[i][j + 1] + -1 * pixels[i + 1][j - 1] + 1 * pixels[i + 1][j + 1]) / 8;

			float SYvalue = (-1 * pixels[i - 1][j - 1] + 1 * pixels[i - 1][j + 1] + -2 * pixels[i - 1][j] + 2 * pixels[i + 1][j] + -1 * pixels[i + 1][j - 1] + 1 * pixels[i + 1][j + 1]) / 8;
